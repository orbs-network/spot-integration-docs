...
const ORDER_HISTORY_REQUEST_CODE = `export async function fetchOrderHistory() {
  const search = new URLSearchParams({
    swapper: "0x5555555555555555555555555555555555555555",
    chainId: "137",
    // Always send the submission partner ID as exchange, not an adapter address.
    exchange: "external",
  });
...
const REFERENCE_EXAMPLES_BASE = {
  ...
  "advanced-orders-direct:fetch-order-sink-orders": {
    ...
    "files": [
  {
    "code": "import type { Address, FetchOrdersResponse } from \"./order-types\";\n\nexport async function fetchOrders({ account, chainId, exchange }: {\n  account: Address;\n  chainId: number;\n  exchange: string;\n}): Promise<FetchOrdersResponse> {\n  if (!exchange?.trim()) throw new Error(\"Exchange partner ID is required to fetch order history\");\n  const query = new URLSearchParams({\n    swapper: account,\n    chainId: String(chainId),\n    exchange,\n  });\n  const response = await fetch(`https://order-sink-v2.orbs.network/orders?${query}`, {\n    headers: { Accept: \"application/json\" },\n  });\n  if (!response.ok) throw new Error(`Failed to fetch orders (${response.status})`);\n  return (await response.json()) as FetchOrdersResponse;\n}\n",
    "language": "typescript",
  ...
  ADVANCED_ORDER_CREATE_FILE
    ...
    "help": "Fetch orders for the connected wallet and chain. Always send the submission partner ID using the exchange query parameter, not partner or an adapter address.",
    "label": "Interactive HTTP Reference",
    "purpose": "Load order history for the connected owner and configured partner via exchange.",
